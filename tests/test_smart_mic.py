...
class SmartMicTester(xmostest.Tester):
    ...
    def __init__(self, test, app_name, app_config, num_chans, doa_dir, sample_rate):
...
def do_xvsm_frequency_sweep_test(min_testlevel, board, app_name, app_config, num_chans, sample_rate):
    # Setup the tester which will determine and record the result
    tester = xmostest.CombinedTester(3, SmartMicTester("xvsm_doa_test",
                                                       app_name, app_config,
                                                       num_chans, 'N/A', sample_rate))
    tester.set_min_testlevel(min_testlevel)
    ...
    #TODO The file name should describe the test
    mic_data_file_name = 'recording_frequency_sweep' % rate
        ...
        {'board':'mic_array', 'app':'app_usb_aud_mic_array',
         'app_configs':[
            {'config':'1i2o2_xvsm2000', 'chan_count':8,
             'testlevels':[
                {'level':'smoke', 'doa_dirs':[1], 'sample_rates':[16000]},
                {'level':'nightly', 'doa_dirs':[2, 3, 4, 5, 6], 'sample_rates':[44100]}
             ]
            },
         ]
        },
        ...
        for config in test['app_configs']:
            ...
            for run_type in config['testlevels']:
                ...
                for sr in sample_rates:                      
                    do_xvsm_frequency_sweep_test(min_testlevel, board, app, config_name,
                                      num_chans, sr)                      
