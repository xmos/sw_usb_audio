...
def iterate_fs_and_dut(samp_freqs, adapter_dut, board, config):
    # Swapped the order of the for and the with to workaround MacOS (https://github.com/xmos/sw_usb_audio/issues/312)
    # issue where it seems to hang after changing the clock source between ADAT and Internal after about 4 iterations.
...
def test_adat_input(pytestconfig, board, config):
    ...
    for fs, dut in iterate_fs_and_dut(features["samp_freqs"], adapter_dut, board, config):
